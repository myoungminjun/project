...
<li><img src="./img/banner_1920.jpg" alt="" class="vs_img">
		...
		<div class="content">
			<div class="writePosibleArea">
				<c:if test="${session.getAttribute('cusid') == 'admin'}">
					...
					<tbody>
						...
						<c:forEach items="${boardList }" var="board" varStatus="status">
							<tr>
								...
								<td class="cont"><a
									href="GetBoardCtrl2?num=${board.getNum() }&pageNum=${spage }">${board.getSubject() }</a>
								</td>
         ...
         $("#bList").DataTable();      //#custom_tb는 반드시 table 태그일 것.
			...
			<div id="pageForm" class="pagenum">
				<c:if test="${startPage != 1 }">
					<a href='../GetBoardListCtr2l?page=${startPage-1 }'>[ 이전 ]</a>
				</c:if>
				<c:forEach var="pageNum" begin="${startPage }" end="${endPage }">
                   ...
                   ${pageNum }&nbsp;
					...
					<c:if test="${pageNum != spage }">
						<a href='../GetBoardListCtrl2?page=${pageNum }'>${pageNum }&nbsp;</a>
					</c:if>
				</c:forEach>
				<c:if test="${endPage != maxPage }">
					<a href='../GetBoardListCtrl2?page=${endPage+1 }'>[다음]</a>
				</c:if>
			</div>
		</div>
	</div>
